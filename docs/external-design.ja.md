...
対応する上位項目：[1. 背景と目的](requirements.ja.md#req-purpose)、[2. 利用者と利用場面](requirements.ja.md#req-users)、[6. 外部インターフェース要件](requirements.ja.md#req-interfaces)、[7.2 開発上の制約](requirements.ja.md#req-constraints)。
...
| --- | --- | --- | --- | --- |
| <a name="use-python"></a>[Pythonから使う](#use-python) | 計算ライブラリとPythonプログラムまたはノートブック | 研究者、または利用者が書いたプログラム | ライブラリを導入した環境 | [手動で計算する](requirements.ja.md#user-python)、[I-01](requirements.ja.md#req-i-01) |
| <a name="use-external"></a>[外部エージェントから使う](#use-external) | CodexやClaude CodeなどのMCPクライアントと、MCPサーバーの起動設定 | 外部エージェント | MCPサーバーを起動した環境 | [外部エージェントに計算を依頼する](requirements.ja.md#user-external)、[I-02](requirements.ja.md#req-i-02) |
| <a name="use-agent"></a>[付属エージェントから使う](#use-agent) | 計算ライブラリ、付属エージェント、言語モデルの接続設定 | 付属エージェント | 付属エージェントを起動した環境 | [付属エージェントで探索する](requirements.ja.md#user-agent)、[I-03](requirements.ja.md#req-i-03)、[C-01](requirements.ja.md#req-c-01) |
...
対応する上位項目：[4. 機能要件](requirements.ja.md#req-functions)、[6. 外部インターフェース要件](requirements.ja.md#req-interfaces)。
...
| <a name="operation-ml"></a>[機械学習で候補を探す](#operation-ml) | 実行番号、探索条件、学習設定 | 数値候補、厳密な数式への変換結果、変換できた候補の検証結果、終了理由 | [F-06](requirements.ja.md#req-f-06)、[N-04](requirements.ja.md#req-n-04) |
| <a name="operation-couplings"></a>[Lax接続の存在条件を結合定数について求める](#operation-couplings) | 実行番号、模型族、結合定数と候補の探索範囲、探索方法 | Lax接続を得られた結合定数の条件、その接続、検証済み・未解決・未探索の範囲 | [F-12](requirements.ja.md#req-f-12)、[保存する情報：模型族の探索結果](requirements.ja.md#data-family)、[伝達事項：Lax接続の存在条件の探索](requirements.ja.md#handoff-couplings) |
| <a name="operation-results"></a>[進捗や結果を読む](#operation-results) | 実行番号、必要に応じて計算番号や取得する結果の指定 | 進捗、残予算、候補、検証結果、根拠となる数式、履歴 | [F-08](requirements.ja.md#req-f-08)、[F-13](requirements.ja.md#req-f-13)、[保存する情報：実行条件と履歴](requirements.ja.md#data-history) |
| <a name="operation-control"></a>[停止・条件変更・再開を行う](#operation-control) | 実行番号、操作、変更する条件と理由 | 操作の受付状況、変更前後の条件、保存済みの進捗、再開する計算 | [F-07](requirements.ja.md#req-f-07)、[F-10](requirements.ja.md#req-f-10)、[F-13](requirements.ja.md#req-f-13)、[保存する情報：人の介入](requirements.ja.md#data-intervention)、[伝達事項：実行中の人の介入](requirements.ja.md#handoff-intervention) |
| <a name="operation-records"></a>[記録を出力・再検算する](#operation-records) | 実行番号、出力対象、または保存された記録 | 別環境へ渡せる入力・結果・実行条件、再検算の結果と元の結果との差 | [F-08](requirements.ja.md#req-f-08)、[F-09](requirements.ja.md#req-f-09)、[R-03](requirements.ja.md#req-r-03)、[受入確認：保存した結果の再検算](requirements.ja.md#accept-recheck) |
...
対応する上位項目：[3. 対象範囲](requirements.ja.md#req-scope)、[7. 非機能要件と制約](requirements.ja.md#req-quality)、[7.1 非機能要件](requirements.ja.md#req-nonfunctional)、[8. 受入方針](requirements.ja.md#req-acceptance)、[8.1 最初の受入範囲](requirements.ja.md#req-initial-acceptance)、[N-01](requirements.ja.md#req-n-01)、[N-02](requirements.ja.md#req-n-02)、[受入確認：既知のLax接続](requirements.ja.md#accept-known-lax)、[受入確認：未判定の扱い](requirements.ja.md#accept-undecided)。

候補の検証では、次の3項目をそれぞれ判定する。

| 検査 | 確認する内容 | 結果に添える根拠 | 対応する上位項目 |
| --- | --- | --- | --- |
| <a name="check-curvature"></a>[零曲率条件](#check-curvature) | 運動方程式と模型の恒等式の下で、候補の曲率がスペクトルパラメータの許容範囲全体で零になるか | 曲率の計算と使用した条件。不成立なら消えない式 | [F-03](requirements.ja.md#req-f-03) |
| <a name="check-equations"></a>[運動方程式全体の回収](#check-equations) | 零曲率条件と模型の恒等式から、対象模型の運動方程式全体が得られるか | 検査対象の全方程式と対応関係。不足がある場合は得られない方程式 | [F-04](requirements.ja.md#req-f-04)、[受入確認：運動方程式を表さない候補](requirements.ja.md#accept-missing-equations) |
| <a name="check-essential"></a>[スペクトルパラメータ依存の非自明性](#check-essential) | 許容する局所ゲージ変換によって、その依存を除去できないか | 許容する変換の範囲と除去できない根拠。除去可能なら具体的な変換 | [F-11](requirements.ja.md#req-f-11)、[伝達事項：非自明性の検証条件](requirements.ja.md#handoff-essential)、[受入確認：除去可能なスペクトルパラメータ依存](requirements.ja.md#accept-removable-parameter) |
...
対応する上位項目：[F-07](requirements.ja.md#req-f-07)、[F-10](requirements.ja.md#req-f-10)、[F-13](requirements.ja.md#req-f-13)、[N-03](requirements.ja.md#req-n-03)、[C-01](requirements.ja.md#req-c-01)、[R-03](requirements.ja.md#req-r-03)。
...
対応する上位項目：[N-02](requirements.ja.md#req-n-02)、[N-04](requirements.ja.md#req-n-04)、[N-05](requirements.ja.md#req-n-05)、[伝達事項：計算ツールの追加](requirements.ja.md#handoff-tools)。
